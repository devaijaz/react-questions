...
export const ContextProvider:FC<PropTypes> = ({children}) => {
   ...
   */
  useEffect(()=> {
    fetch('/tickets.json').then(response=> response.json()).then((data:Ticket[])=> {
   ...
   */
  useEffect(()=> {
    setTotal(_ => tickets.reduce((accumulator, row) => (row.quantity * row.unitPrice) + accumulator, 0));
  }, [tickets])

  const addTicket = (data: Ticket)=> {
    setTickets(oldTickets=> {
      return [...oldTickets, {...data, id: uuid()}]
    });
  }
